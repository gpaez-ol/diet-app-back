...
namespace AlgoFit.WebAPI.Logic
{
    public class SessionLogic
    {
        ...
        public SessionLogic(RepositoryManager repositoryManager, IMapper mapper)
        {
        ...
        public async Task<ProfileDTO> Login(LoginDTO login, HttpResponse response)
        {
        ...
        public List<Claim> GetUserClaims(User user)
        {
            ...
            claims.Add(new Claim("user_type", EnumHelper.GetEnumText(user.Type)));
            return claims;
        }
        public void SignToken(HttpResponse response, User user) 
        {
            List<Claim> claims = GetUserClaims(user);
            TokenHandler.SetJWTCookie(response, claims);
        }
    }
}
